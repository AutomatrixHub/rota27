...
function clean(value: unknown, max = 500) { return String(value ?? "").replace(/\u0000/g, "").replace(/[\r\t]+/g, " ").replace(/\s{2,}/g, " ").trim().slice(0, max); }
...
function parseBirthDate(value: unknown) {
  ...
  if (y < 1900 || mo < 1 || mo > 12 || d < 1 || d > 31) return ""; const dt = new Date(Date.UTC(y, mo - 1, d)); if (dt.getUTCFullYear() !== y || dt.getUTCMonth() !== mo - 1 || dt.getUTCDate() !== d) return "";
  const iso = `${String(y).padStart(4,"0")}-${String(mo).padStart(2,"0")}-${String(d).padStart(2,"0")}`; const now = new Date(); const today = `${now.getUTCFullYear()}-${String(now.getUTCMonth()+1).padStart(2,"0")}-${String(now.getUTCDate()).padStart(2,"0")}`; return iso <= today ? iso : "";
}
...
function extractMessageText(message: any) {
  ...
  if (type === "interactive") return { type, text: clean(message?.interactive?.button_reply?.title || message?.interactive?.list_reply?.title || "[Resposta interativa]", 700) };
  if (type === "image") { const caption = clean(message?.image?.caption, 600); return { type, text: caption ? `[Imagem recebida] ${caption}` : "[Imagem recebida]" }; }
  if (type === "video") { const caption = clean(message?.video?.caption, 600); return { type, text: caption ? `[Vídeo recebido] ${caption}` : "[Vídeo recebido]" }; }
  if (type === "document") { const filename = clean(message?.document?.filename, 160), caption = clean(message?.document?.caption, 400); return { type, text: `[Documento recebido${filename ? `: ${filename}` : ""}]${caption ? ` ${caption}` : ""}` }; }
...
Deno.serve(async (req: Request) => {
  ...
  async function applyDeliveryStatus(status: any) {
    ...
    const nextPayload = { ...(row.payload && typeof row.payload === "object" ? row.payload : {}), delivery: { status: deliveryStatus, timestamp, receivedAt: new Date().toISOString(), recipientId: clean(status?.recipient_id, 80) || null, conversationId: clean(status?.conversation?.id, 180) || null, conversationOrigin: clean(status?.conversation?.origin?.type, 80) || null, pricingCategory: clean(status?.pricing?.category, 80) || null, pricingModel: clean(status?.pricing?.pricing_model, 80) || null, billable: typeof status?.pricing?.billable === "boolean" ? status.pricing.billable : null, error: errorText || null } };
    const patch: Record<string, unknown> = { payload: nextPayload, updated_at: new Date().toISOString() };
    if (deliveryStatus === "failed") { patch.status = "failed"; patch.last_error = errorText || "Falha de entrega reportada pela Meta."; }
    else if (["sent", "delivered", "read"].includes(deliveryStatus) && row.status !== "failed") { patch.status = "sent"; patch.last_error = null; }
    const { error: updateError } = await db.from("whatsapp_message_log").update(patch).eq("event_id", row.event_id); if (updateError) throw new Error(`Falha ao salvar status de entrega: ${updateError.message}`);
  ...
  async function latestClient(clientId: string, senderPhone: string) {
    if (clientId) { const { data, error } = await db.from("rota27_sync_events").select("payload,entity_id,seq").eq("store_id", storeId).eq("event_type", "client_upsert").eq("entity_id", clientId).order("seq", { ascending: false }).limit(1).maybeSingle(); if (error) throw new Error(`Falha ao localizar cliente: ${error.message}`); if (data?.payload?.client) return { ...data.payload.client, id: clean(data.payload.client.id || data.entity_id, 160) }; }
    const { data, error } = await db.from("rota27_sync_events").select("payload,entity_id,seq").eq("store_id", storeId).eq("event_type", "client_upsert").order("seq", { ascending: false }).limit(1000); if (error) throw new Error(`Falha ao localizar cliente por telefone: ${error.message}`); const seen = new Set<string>(); for (const row of data || []) { const c = row?.payload?.client; if (!c) continue; const id = clean(c.id || row.entity_id, 160); if (!id || seen.has(id)) continue; seen.add(id); if (normalizePhone(c.whatsappPhone || c.phone || "") === senderPhone) return { ...c, id }; } return null;
  }
  async function persistBirthday(original: any, senderPhone: string, birthDate: string, metaMessageId: string) {
    const clientId = clean(original?.payload?.clientId || "", 160), client = await latestClient(clientId, senderPhone); if (!client?.id) throw new Error("Cliente da campanha não foi localizado no cadastro sincronizado.");
    const updatedAt = Date.now();
    const next = { ...client, birthDate, birthDateUpdatedAt: updatedAt, relationshipMarketingOptIn: true, relationshipMarketingOptInAt: Number(client.relationshipMarketingOptInAt || updatedAt), relationshipMarketingOptOutAt: 0, relationshipMarketingConsentSource: "birth_date_reply_whatsapp_v02568", eventMarketingOptIn: true, eventMarketingOptInAt: Number(client.eventMarketingOptInAt || client.relationshipMarketingOptInAt || updatedAt), eventMarketingOptOutAt: 0, eventMarketingConsentSource: "birth_date_reply_whatsapp_v02568" };
    const eventId = `birthday_reply::${metaMessageId}`;
    const { error } = await db.from("rota27_sync_events").insert({ store_id: storeId, event_id: eventId, device_id: "backend-whatsapp-birthday", event_type: "client_upsert", entity_id: client.id, payload: { client: next }, app_version: "0.25.68", client_created_at: new Date().toISOString() });
    if (error && error.code !== "23505") throw new Error(`Falha ao salvar aniversário: ${error.message}`); return next;
  }
  ...
  const messages: Array<{ value: any; message: any }> = []; const statuses: any[] = [];
  for (const entry of Array.isArray(body?.entry) ? body.entry : []) for (const change of Array.isArray(entry?.changes) ? entry.changes : []) { if (change?.field !== "messages") continue; const value = change?.value || {}; if (clean(value?.metadata?.phone_number_id, 120) !== clean(phoneNumberId, 120)) continue; for (const status of Array.isArray(value?.statuses) ? value.statuses : []) statuses.push(status); for (const message of Array.isArray(value?.messages) ? value.messages : []) messages.push({ value, message }); }

  let statusProcessed = 0, statusIgnored = 0, statusFailures = 0;
  for (const status of statuses) { try { const result = await applyDeliveryStatus(status); if (result.matched) statusProcessed++; else statusIgnored++; } catch (error) { statusFailures++; console.error("[Rota27] Falha ao processar status Meta:", error); } }
  if (!messages.length) { if (statusFailures) return json(500, { ok: false, processed: 0, ignored: 0, failures: statusFailures, statusProcessed, statusIgnored, edgeVersion: EDGE_VERSION }); return json(200, { ok: true, processed: 0, ignored: 0, failures: 0, statusProcessed, statusIgnored, edgeVersion: EDGE_VERSION }); }

  let processed = 0, ignored = 0, failures = 0;
  for (const { message } of messages) {
    const metaMessageId = clean(message?.id, 300), sender = normalizePhone(message?.from || ""), replyToMessageId = clean(message?.context?.id, 300), extracted = extractMessageText(message), metaTimestamp = Number(message?.timestamp || 0) || null; if (!metaMessageId || !validPhone(sender)) { ignored++; continue; }
    const { data: existing } = await db.from("rota27_whatsapp_inbound").select("status").eq("meta_message_id", metaMessageId).limit(1).maybeSingle(); if (existing?.status === "forwarded" || existing?.status === "ignored") { ignored++; continue; }
    await writeInbound({ meta_message_id: metaMessageId, sender_phone: sender, reply_to_message_id: replyToMessageId || null, message_type: extracted.type, message_text: extracted.text, status: "received", reason: null, meta_timestamp: metaTimestamp, updated_at: new Date().toISOString() });
    try {
      let original = replyToMessageId ? await originalMessage(replyToMessageId, sender) : null; if (!original || original?.payload?.campaign !== BIRTHDAY_CAMPAIGN) { const birthdayOriginal = await recentBirthdayMessage(sender); if (birthdayOriginal) original = birthdayOriginal; }
      if (original?.payload?.campaign === BIRTHDAY_CAMPAIGN) {
        const birthDate = parseBirthDate(extracted.text), clientId = clean(original?.payload?.clientId || "", 160), client = await latestClient(clientId, sender);
        if (birthDate) { const saved = await persistBirthday(original, sender, birthDate, metaMessageId); await sendSessionText(sender, `Obrigado, ${clean(saved?.name || "Cliente", 80)}! Sua data de nascimento foi atualizada para ${formatBirthDate(birthDate)}.`, `birthday_ack::${metaMessageId}`, saved); await writeInbound({ meta_message_id: metaMessageId, sender_phone: sender, reply_to_message_id: replyToMessageId || original.wa_message_id || null, message_type: extracted.type, message_text: extracted.text, command_id: original.command_id, customer_name: original.customer_name, command_label: original.command_label, status: "forwarded", reason: `Data de nascimento salva automaticamente: ${birthDate}.`, meta_timestamp: metaTimestamp, processed_at: new Date().toISOString(), updated_at: new Date().toISOString() }); processed++; continue; }
        await sendSessionText(sender, "Não consegui identificar a data. Por favor, responda somente com a data de nascimento no formato DD/MM/AAAA. Exemplo: 15/09/1985.", `birthday_retry::${metaMessageId}`, client || { id: clientId, name: original.customer_name }); await writeInbound({ meta_message_id: metaMessageId, sender_phone: sender, reply_to_message_id: replyToMessageId || original.wa_message_id || null, message_type: extracted.type, message_text: extracted.text, command_id: original.command_id, customer_name: original.customer_name, command_label: original.command_label, status: "ignored", reason: "Resposta da campanha de aniversário sem data válida; orientação reenviada.", meta_timestamp: metaTimestamp, processed_at: new Date().toISOString(), updated_at: new Date().toISOString() }); ignored++; continue;
      }
      if (!replyToMessageId) { await writeInbound({ meta_message_id: metaMessageId, sender_phone: sender, reply_to_message_id: null, message_type: extracted.type, message_text: extracted.text, status: "ignored", reason: "Mensagem sem contexto de resposta.", meta_timestamp: metaTimestamp, processed_at: new Date().toISOString(), updated_at: new Date().toISOString() }); ignored++; continue; }
      if (!original) { await writeInbound({ meta_message_id: metaMessageId, sender_phone: sender, reply_to_message_id: replyToMessageId, message_type: extracted.type, message_text: extracted.text, status: "ignored", reason: "Mensagem original não pertence a este cliente/Rota 27.", meta_timestamp: metaTimestamp, processed_at: new Date().toISOString(), updated_at: new Date().toISOString() }); ignored++; continue; }
      const manager = await latestManager(); if (!manager.enabled || !manager.phone) { await writeInbound({ meta_message_id: metaMessageId, sender_phone: sender, reply_to_message_id: replyToMessageId, message_type: extracted.type, message_text: extracted.text, command_id: original.command_id, customer_name: original.customer_name, command_label: original.command_label, status: "ignored", reason: "WhatsApp do gerente desativado ou inválido.", meta_timestamp: metaTimestamp, processed_at: new Date().toISOString(), updated_at: new Date().toISOString() }); ignored++; continue; }
      if (sender === manager.phone) { await writeInbound({ meta_message_id: metaMessageId, sender_phone: sender, reply_to_message_id: replyToMessageId, message_type: extracted.type, message_text: extracted.text, command_id: original.command_id, customer_name: original.customer_name, command_label: original.command_label, manager_phone: manager.phone, manager_name: manager.name, status: "ignored", reason: "Resposta originada do próprio gerente; loop bloqueado.", meta_timestamp: metaTimestamp, processed_at: new Date().toISOString(), updated_at: new Date().toISOString() }); ignored++; continue; }
