...
const EDGE_VERSION = "rota27-event-campaign-v2";
...
function eventKey(value: unknown) { return clean(value, 120).replace(/[^a-zA-Z0-9_-]/g, "_").replace(/_+/g, "_").slice(0, 100); }
...
async function graphJson(url: string, accessToken: string, init: RequestInit = {}) {
  ...
  if (!response.ok) {
    ...
    const message = clean(err.message || `Meta HTTP ${response.status}`, 600);
    const title = clean(err.error_user_title || "", 300);
    const userMsg = clean(err.error_user_msg || "", 900);
    const details = clean(err?.error_data?.details || "", 900);
    const parts = [title, userMsg, details].filter(Boolean);
    const error: any = new Error(parts.length ? `${message} | ${parts.join(" | ")}` : message);
    error.metaCode = err.code || null;
    error.metaSubcode = err.error_subcode || null;
    error.fbtraceId = clean(err.fbtrace_id || "", 200);
    throw error;
  }
  return data;
}
...
async function submitTemplate(accessToken: string, waba: string, graphVersion: string) {
  const existing = await getTemplate(accessToken, waba, graphVersion);
  if (existing.found) return { existing: true, ...existing };

  const text = "Olá, {{1}}! A Rota 27 Bodega preparou um convite especial para você: {{2}}. Data e horário: {{3}}. Detalhes: {{4}}. Esperamos você no Rota 27!";
  const payload = {
    name: TEMPLATE_NAME,
    ...
    components: [
      {
        ...
        example: {
          body_text: [[
            "Marcos",
            "Degustação de Costela com Aipim",
            "28/08 às 18:00",
            "Costela com aipim, cachaças e chopp Brahma bem gelado. Vem para o Rota 27!",
          ]],
        },
      },
      { type: "FOOTER", text: "Convites enviados com sua autorização." },
    ],
  };
...
Deno.serve(async (req: Request) => {
  ...
  async function latestClients() {
    ...
    for (const row of data || []) {
      ...
      for (const key of ["eventMarketingOptIn", "eventMarketingOptInAt", "eventMarketingOptOutAt", "eventMarketingConsentSource"]) if (c[key] === undefined && old[key] !== undefined) merged[key] = old[key];
      map.set(id, merged);
    }
    return [...map.values()];
  }

  async function campaignRows(id: string) {
    if (!id) return [];
    const { data, error } = await db.from("whatsapp_message_log").select("event_id,status,wa_message_id,last_error,sent_at,updated_at,customer_name,phone,payload").like("event_id", `${CAMPAIGN}::${id}::%`).order("updated_at", { ascending: false }).limit(1000);
  ...
  if (action === "submit_template") {
    try {
      const template = await submitTemplate(accessToken, waba, graphVersion);
      return json(200, { ok: true, edgeVersion: EDGE_VERSION, templateName: TEMPLATE_NAME, template });
    } catch (error: any) {
      return json(502, { ok: false, error: clean(error?.message || "Falha ao submeter template.", 1400), metaCode: error?.metaCode || null, metaSubcode: error?.metaSubcode || null, fbtraceId: error?.fbtraceId || null, edgeVersion: EDGE_VERSION });
    }
  }

  if (action === "status") {
    let template: any, templateError: string | null = null;
    try { template = await getTemplate(accessToken, waba, graphVersion); }
    catch (error) { template = { found: false, status: "ERROR" }; templateError = clean(error instanceof Error ? error.message : "Falha ao consultar template.", 1200); }
    const clients = normalizedAudience(await latestClients());
    const requestedIds = new Set((Array.isArray(body?.targetClientIds) ? body.targetClientIds : []).map((x: any) => clean(x, 160)).filter(Boolean));
    const scoped = requestedIds.size ? clients.filter(c => requestedIds.has(c.id)) : clients;
    const eligible = scoped.filter(c => validPhone(c.phone) && c.marketingOptIn);
    const eKey = eventKey(body?.eventId);
    const logs = eKey ? await campaignRows(eKey) : [];
    return json(200, { ok: true, edgeVersion: EDGE_VERSION, templateName: TEMPLATE_NAME, template, templateError, counts: { clients: clients.length, marketingOptIn: clients.filter(c => validPhone(c.phone) && c.marketingOptIn).length, selected: scoped.length, eligible: eligible.length }, campaign: { eventId: eKey || null, sent: logs.filter((r: any) => r.status === "sent").length, failed: logs.filter((r: any) => r.status === "failed").length, total: logs.length } });
  }
  ...
  const eKey = eventKey(event.id);
  const title = clean(event.title, 120);
  const eventDate = clean(event.eventDate, 20);
  const eventTime = clean(event.eventTime, 20);
  const description = clean(event.description, 260);
  const callToAction = clean(event.callToAction, 180);
  if (!eKey || !title || !eventDate || !eventTime) return json(400, { ok: false, error: "Evento incompleto: informe id, título, data e horário." });

  let template: any;
  try { template = await getTemplate(accessToken, waba, graphVersion); }
  catch (error) { return json(502, { ok: false, error: clean(error instanceof Error ? error.message : "Falha ao consultar template.", 1200), edgeVersion: EDGE_VERSION }); }
  ...
  const clients = normalizedAudience(await latestClients());
  const targets = clients.filter(c => selectedIds.has(c.id) && validPhone(c.phone) && c.marketingOptIn).slice(0, 100);
  ...
  for (const target of targets) {
    const eventId = `${CAMPAIGN}::${eKey}::${target.id}`;
    ...
    try {
      ...
      const errorText = clean(error?.message || "Falha ao enviar.", 1200);
