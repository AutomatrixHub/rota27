...
async function readExisting(eventId: string) {
  ...
  const endpoint =
    `${url}/rest/v1/whatsapp_message_log?event_id=eq.${encodeURIComponent(eventId)}` +
    "&select=status,wa_message_id,updated_at&limit=1";
...
Deno.serve(async (req: Request) => {
  ...
  const commandLabel = safeText(body?.commandLabel || "Comanda", 120);
  const customerName = safeText(body?.customerName || "Cliente", 120);
  ...
  if (!eventId || !commandId) {
    return json(400, { ok: false, error: "eventId e commandId são obrigatórios." });
  }
  if (!consent) {
    return json(400, { ok: false, error: "Consentimento não confirmado." });
  }
  if (!validPhone(phone)) {
    return json(400, { ok: false, error: "Número de WhatsApp inválido." });
  }
  if (!items.length) {
    return json(400, { ok: false, error: "Nenhum item para enviar." });
  }
  if (!Number.isFinite(total) || total < 0) {
    return json(400, { ok: false, error: "Total inválido." });
  }

  const existing = await readExisting(eventId);
  if (existing?.status === "sent") {
    return json(200, {
      ok: true,
      duplicate: true,
      messageId: existing.wa_message_id || null,
    });
  }
  ...
  const normalizedItems = items
    .map((item: any) => {
      ...
      const name = safeText(item?.name || "Produto", 160);
  ...
  const itemLines = normalizedItems.map((item: any) => {
    const sign = item.delta > 0 ? "+" : "−";
    const subtotal = item.quantity * item.unitPrice;
    return `${sign} ${item.quantity}× ${item.name} — ${moneyBRL(subtotal)}`;
  });
  ...
  const templateName =
    Deno.env.get("WHATSAPP_TEMPLATE_NAME") || "atualizacao_comanda_rota27";
  const templateLang =
  ...
  try {
    ...
    if (!metaResponse.ok) {
      const errorText = safeText(
        metaData?.error?.message || `Meta HTTP ${metaResponse.status}`,
        500,
      );
      ...
      return json(502, { ok: false, error: errorText });
    }
    ...
    const errorText = safeText(
      error instanceof Error ? error.message : "Falha ao chamar a Meta.",
      500,
    );
