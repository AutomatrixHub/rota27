const CACHE_NAME = 'rota27-comandas-v0.25.87-r1';
const APP_SHELL = [
...
self.addEventListener('message',event=>{
  ...
  if(type==='ROTA27_GET_RELEASE')event.ports?.[0]?.postMessage({version:'0.25.87',cacheName:CACHE_NAME});
});
self.addEventListener('fetch',event=>{if(event.request.method!=='GET')return;const url=new URL(event.request.url);if(url.pathname.endsWith('/assets/roadmap-loader.js')){event.respondWith(caches.open(CACHE_NAME).then(cache=>cache.match('./assets/roadmap-loader.js')).then(cached=>cached||fetch(event.request,{cache:'reload'})));return;}event.respondWith(fetch(event.request).then(response=>{const copy=response.clone();caches.open(CACHE_NAME).then(cache=>cache.put(event.request,copy));return response;}).catch(()=>caches.match(event.request,{ignoreSearch:true}).then(cached=>cached||caches.match('./index.html'))));});
