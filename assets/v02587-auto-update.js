...
(function(){
  'use strict';
  if(window.Rota27V02587AutoUpdate)return;

  const VERSION='0.25.87';
  ...
  function currentVersion(){return clean(document.querySelector('meta[name="rota27-release-version"]')?.content||VERSION,40)||VERSION;}
  ...
  function toast(message,error=false){
    ...
    console[error?'error':'info']('[Rota27 auto update]',message);
  }
  function syncConfig(){
    try{
  ...
  async function controlApi(action,extra={}){
    ...
    const response=await fetch(controlUrl(cfg),{
      ...
      body:JSON.stringify({action,storeId:cfg.storeId,deviceId:cfg.deviceId,deviceName:cfg.deviceName||'Aparelho',appVersion:currentVersion(),afterSeq:cfg.cursor||0,...extra})
    });
    let data={};try{data=await response.json();}catch{}
    if(!response.ok||data?.ok===false){const err=new Error(clean(data?.error||`Falha HTTP ${response.status}`,300));err.code=clean(data?.code,60);throw err;}
    return data||{};
  }
  ...
  function scheduleReload(){
    ...
    const attempt=()=>{
      reloadTimer=null;
      if(!safeToReload()){
        if(!waitingNoticeShown){waitingNoticeShown=true;toast('Atualização pronta. Ela será aplicada automaticamente ao concluir a operação atual.');}
        reloadTimer=setTimeout(attempt,1500);return;
      }
      storePendingAck();
      location.reload();
    };
  ...
  function watchWorker(worker){
    if(!worker)return;
    const onState=()=>{if(worker.state==='activated'){updateArmed=true;scheduleReload();}};
  ...
  async function armServiceWorkerUpdate(targetVersion){
    ...
    if(!reg)return false;
    updateArmed=true;
    const onFound=()=>watchWorker(reg.installing);
    reg.addEventListener('updatefound',onFound,{once:true});
    if(reg.waiting){try{reg.waiting.postMessage({type:'ROTA27_SKIP_WAITING'});}catch{}watchWorker(reg.waiting);}
    if(reg.installing)watchWorker(reg.installing);
    try{await reg.update();}catch(err){console.warn('[Rota27 v0.25.87] SW update:',err);}
    if(reg.waiting){try{reg.waiting.postMessage({type:'ROTA27_SKIP_WAITING'});}catch{}watchWorker(reg.waiting);}
    if(reg.installing)watchWorker(reg.installing);
    setTimeout(async()=>{
      const info=await probeController();
      if(info?.version&&compareVersion(info.version,targetVersion)>=0)scheduleReload();
    },1800);
    return true;
  }
  async function acknowledgeRemote(requestedAt){
    ...
    try{
      await controlApi('agent',{ackUpdateRequestAt:requestedAt});
      try{localStorage.removeItem(ACK_KEY);}catch{}
    }catch(err){console.warn('[Rota27 v0.25.87] ACK update:',err);}
  }
  ...
  async function handleRemoteRequest(requestedAt,targetVersion=''){
    ...
    if(pendingRemote.targetVersion&&compareVersion(currentVersion(),pendingRemote.targetVersion)>=0){await acknowledgeRemote(ts);pendingRemote=null;return;}
    await checkForUpdate('remote',pendingRemote.targetVersion);
  }

  function pending(req,ack){if(!req)return false;const r=new Date(req).getTime(),a=ack?new Date(ack).getTime():0;return Number.isFinite(r)&&r>a;}
  async function remoteAgentOnce(){
    ...
    remoteBusy=true;
    try{
      const data=await controlApi('agent');
      if(data?.requestedUpdateAt)await handleRemoteRequest(String(data.requestedUpdateAt),String(data.requestedUpdateVersion||''));
      if(document.getElementById('v02585DeviceWrap')?.classList.contains('open'))setTimeout(refreshDeviceUi,100);
    }catch(err){if(!['device_inactive','device_not_registered'].includes(String(err?.code||'')))console.warn('[Rota27 v0.25.87] update agent:',err);}
  ...
  async function refreshDeviceUi(){
    ...
    try{
      ...
      augmentDeviceRows();
    }catch(err){console.warn('[Rota27 v0.25.87] update UI:',err);}
  }
  function augmentDeviceRows(){
    ...
    list.querySelectorAll('.v02585-device-row[data-device-row]').forEach(row=>{
      ...
      const main=row.querySelector('.v02585-device-main');
      let versionLine=row.querySelector('.v02587-version-line');
      if(!versionLine){versionLine=document.createElement('div');versionLine.className='v02587-version-line';main?.appendChild(versionLine);}
      const updatePending=pending(device.requested_update_at,device.update_request_ack_at);
      versionLine.textContent=`Versão instalada: ${device.app_version?`v${device.app_version}`:'desconhecida'}${updatePending?` • atualização para v${device.requested_update_version||currentVersion()} pendente`:''}`;
      const menu=row.querySelector('.v02585-menu-pop');
      if(menu&&!row.classList.contains('retired')&&!row.classList.contains('removed')&&id!==currentDeviceId&&!menu.querySelector('[data-v02587-request-update]')){
        const btn=document.createElement('button');btn.type='button';btn.dataset.v02587RequestUpdate=id;btn.textContent='Solicitar atualização';menu.prepend(btn);
      }
    });
  }
  async function requestDeviceUpdate(id){
    ...
    try{await controlApi('request_update',{targetDeviceId:id,targetVersion:currentVersion()});toast('Atualização solicitada. O aparelho atualizará quando estiver ativo.');await refreshDeviceUi();}
    catch(err){toast(err?.message||'Não foi possível solicitar a atualização.',true);}
  }
  function watchDeviceList(){
    const list=document.getElementById('v02585DeviceList');if(!list||listObserver)return;
    listObserver=new MutationObserver(()=>setTimeout(refreshDeviceUi,80));listObserver.observe(list,{childList:true});
  }

  document.addEventListener('click',event=>{
    const btn=event.target.closest?.('[data-v02587-request-update]');
    if(btn){event.preventDefault();event.stopPropagation();const id=clean(btn.dataset.v02587RequestUpdate,120);btn.closest('.v02585-device-row')?.classList.remove('menu-open');requestDeviceUpdate(id);return;}
    if(event.target.closest?.('#v02585OpenDevices,#v02585RefreshDevices'))setTimeout(()=>{watchDeviceList();refreshDeviceUi();},150);
  },true);
  document.addEventListener('change',event=>{if(event.target?.id==='v02585ShowRemoved')setTimeout(refreshDeviceUi,150);});
  navigator.serviceWorker?.addEventListener?.('controllerchange',()=>{if(updateArmed)scheduleReload();});
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'){setTimeout(()=>checkForUpdate('visible'),250);setTimeout(remoteAgentOnce,400);}});
  window.addEventListener('online',()=>{setTimeout(()=>checkForUpdate('online'),250);setTimeout(remoteAgentOnce,500);});
  window.addEventListener('rota27:test-mode-changed',()=>{if(!inTestMode()){setTimeout(()=>checkForUpdate('test-exit'),500);setTimeout(remoteAgentOnce,700);}});

  function start(){
    watchDeviceList();
    setTimeout(acknowledgeStored,700);
    setTimeout(()=>checkForUpdate('startup'),1200);
    setTimeout(remoteAgentOnce,1500);
  ...
  window.Rota27V02587AutoUpdate={version:VERSION,check:checkForUpdate,handleRemoteRequest,refresh:refreshDeviceUi};
})();
