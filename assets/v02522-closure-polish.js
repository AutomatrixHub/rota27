/* Rota 27 v0.25.22-r2 — render determinístico da tela Fechamentos */
(function(){
  'use strict';
  const VERSION='0.25.22-r2';
  ...
  function renderPolished(syncing=false){
    const wrap=byId('v019HistoryWrap');if(!wrap)return false;
    renderStatus(syncing);renderRows();return true;
  }

  async function syncAndRender(){
    if(syncingUi)return;
    syncingUi=true;renderPolished(true);
    try{await window.Rota27V019?.syncTurnClosures?.();}
    catch{}
    finally{syncingUi=false;renderPolished(false);}
  }

  function openHistory(){
    const wrap=byId('v019HistoryWrap');
    if(!wrap){try{window.Rota27V019?.openTurnHistory?.();}catch{}setTimeout(()=>renderPolished(false),0);return;}
    wrap.classList.add('open');renderPolished(false);
    if(navigator.onLine)syncAndRender();
  }

  function bindCapture(){
    document.addEventListener('click',e=>{
      if(e.target.closest?.('#v019ViewAll')){
        e.preventDefault();e.stopImmediatePropagation();openHistory();return;
      }
      if(e.target.closest?.('#v019HistorySync')){
        e.preventDefault();e.stopImmediatePropagation();syncAndRender();return;
      }
    },true);
  }

  function refreshIfOpen(){if(byId('v019HistoryWrap')?.classList.contains('open')&&!syncingUi)renderPolished(false);}
  ...
  function start(){
    ...
    refreshIfOpen();
    window.Rota27V02522ClosurePolish={version:VERSION,render:renderPolished,openHistory,syncAndRender};
    console.info('[Rota27] v0.25.22-r2 — tela Fechamentos com render determinístico carregada.');
  }
