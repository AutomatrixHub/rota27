...
(function(){
  ...
  function patchReceivable(commandId,businessDate,adminAt){
    const rid=`recv_${commandId}`,rows=read(RECEIVABLE_STORE,[]),idx=Array.isArray(rows)?rows.findIndex(x=>String(x.id)===rid):-1;
    if(idx<0)return;
    rows[idx]={...rows[idx],businessDate,operationalDate:businessDate,administrativeClosedAt:adminAt,updatedAt:Math.max(Number(rows[idx].updatedAt||0),adminAt)};
    write(RECEIVABLE_STORE,rows);
    ...
    if(Array.isArray(out)&&!out.some(x=>String(x.eventId)===eventId)){
      ...
      out.push({eventId,eventType:'receivable_upsert',entityId:rid,payload:{receivable},deviceId:c.deviceId||'local',createdAt:new Date(adminAt).toISOString(),appVersion:VERSION});
      write(RECEIVABLE_OUTBOX,out);
    }
  }

  function queueHistoryCorrection(commandId,historyRecord,adminAt){
    if(!historyRecord)return;
    const c=cfg(),out=Array.isArray(c.outbox)?c.outbox:[],eventId=`history_upsert_businessdate_${commandId}`;
  ...
  function patchHistory(commandId,businessDate,adminAt){
    ...
    state.history[idx]={...state.history[idx],businessDate,operationalDate:businessDate,administrativeClosedAt:adminAt};
    try{save();}catch{}
  ...
  function install(){
    ...
    const wrapped=function(){
      const due=document.getElementById('v14PaymentMethod')?.value==='A receber';
      let command=null,opened=0;
      ...
      if(!due||!command||!opened)return result;
      const adminAt=Date.now(),businessDate=dateKey(opened);
      patchHistory(command.id,businessDate,adminAt);patchReceivable(command.id,businessDate,adminAt);
