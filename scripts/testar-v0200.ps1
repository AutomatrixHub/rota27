...
Write-Host "Rota 27 v0.20.0 - Visao Gerencial" -ForegroundColor Cyan
Write-Host "Branch esperada: feature/v0.20.0-manager-dashboard" -ForegroundColor DarkGray
...
if ($branch -ne "feature/v0.20.0-manager-dashboard") { throw "Branch atual: $branch. Troque para feature/v0.20.0-manager-dashboard antes de abrir a preview." }

$version = (Get-Content (Join-Path $RepoRoot "VERSION") -Raw).Trim()
if ($version -ne "0.20.0") { throw "VERSION local encontrada: $version. Execute git pull --ff-only antes de testar." }

$port = 3023
$listener = Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue
if ($listener) { throw "A porta $port ja esta em uso. Feche o servidor anterior da preview v0.20.0 e execute novamente." }
...
Write-Host "Iniciando servidor dedicado da v0.20.0 na porta $port..." -ForegroundColor Cyan
...
for ($i = 0; $i -lt 30; $i++) {
    ...
    if ($server.HasExited) { throw "O servidor da preview encerrou antes de iniciar. Veja a janela PowerShell aberta para o erro do npx/http-server." }
    $listener = Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue
    if ($listener) { $ready = $true; break }
}
if (-not $ready) { throw "O servidor nao ficou disponivel na porta $port em 30 segundos. Veja a janela PowerShell do servidor." }

$Url = "http://localhost:$port/?preview=v0200"
try {
...
if ($lanIp) {
    $MobileUrl = "http://${lanIp}:$port/?preview=v0200"
