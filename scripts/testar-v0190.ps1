...
Write-Host "Rota 27 v0.19.0 - Fechamento do Turno" -ForegroundColor Cyan
Write-Host "Branch esperada: feature/v0.19.0-turn-close" -ForegroundColor DarkGray
...
if ($branch -ne "feature/v0.19.0-turn-close") { throw "Branch atual: $branch. Troque para feature/v0.19.0-turn-close antes de abrir a preview." }

$version = (Get-Content (Join-Path $RepoRoot "VERSION") -Raw).Trim()
if ($version -ne "0.19.0") { throw "VERSION local encontrada: $version. Execute git pull --ff-only antes de testar." }

$port = 3022
$listener = Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue
if ($listener) { throw "A porta $port ja esta em uso. Feche o servidor anterior da preview v0.19.0 e execute novamente." }
...
Write-Host "Iniciando servidor dedicado da v0.19.0 na porta $port..." -ForegroundColor Cyan
...
$Url = "http://localhost:$port/?preview=v0190"
try {
...
if ($lanIp) {
    $MobileUrl = "http://${lanIp}:$port/?preview=v0190"
